...
function loadPlaces(map) {
  ...
  axios__WEBPACK_IMPORTED_MODULE_0___default.a.get("/api/stores/near?lat=".concat(lat, "&lng=").concat(lng)).then(function (res) {
    var places = res.data;
...
function makeMap(mapDiv) {
  ...
  loadPlaces(map);
  var input = Object(_bling__WEBPACK_IMPORTED_MODULE_1__["$"])('[name="geolocate"]');
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios__WEBPACK_IMPORTED_MODULE_0___default.a.get("/api/search?q=".concat(val)).then(function (res) {
      if (res.data.length) {
        searchResults.innerHTML = dompurify__WEBPACK_IMPORTED_MODULE_1___default.a.sanitize(searchResultsHTML(res.data));
        return;
      }

      searchResults.innerHTML = dompurify__WEBPACK_IMPORTED_MODULE_1___default.a.sanitize("<div class=\"search__result\">No results for ".concat(val, " found!</div>"));
    }).catch(function (err) {
      console.error(err);
    });
  });
